...
void function () {
  ...
  module.exports = emport = function (targetFilename, options, callback) {
    ...
    return async.forEachSeries(basePaths, function (basePath, cb) {
      ...
      var contentsInOrder, dependencies, exports, exportVar, filename, filenamesInOrder, importsAndExports;
      if (null != err)
        return callback(err);
      if (null != options.map) {
      ...
      if (options.output === 'filenames')
        return callback(null, filenamesInOrder);
      contentsInOrder = function (accum$) {
        var contents;
        for (var i$1 = 0, length$1 = filenamesInOrder.length; i$1 < length$1; ++i$1) {
          filename = filenamesInOrder[i$1];
          contents = emportMap[filename].contents;
          accum$.push(filename.match(/\.coffee$/) ? coffee.cs2js(contents, { filename: filename }) : contents);
        }
        return accum$;
      }.call(this, []);
